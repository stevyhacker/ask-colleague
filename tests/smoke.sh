...
assert_contains "$version_out" "0.1.3"
...
if command -v node >/dev/null 2>&1; then
  ...
  assert_contains "$launcher_out" "0.1.3"
  launcher_status=0
...
assert_not_contains "$(cat "$mock_capture/claude.args")" "[--bare]"
...
if command -v node >/dev/null 2>&1; then
  node -e "const p=require('$ROOT_DIR/package.json'); if (p.version !== '0.1.3' || !p.bin || p.bin['ask-colleague'] !== 'bin/colleague.js' || p.bin.colleague !== 'bin/colleague.js') { console.error('package.json mismatch: expected version 0.1.3 with ask-colleague/colleague bin entries pointing at bin/colleague.js, got version ' + p.version); process.exit(1); }"
fi
...
if command -v npm >/dev/null 2>&1; then
  ...
  assert_contains "$npx_version" "0.1.3"
